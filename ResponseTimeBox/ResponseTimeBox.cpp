/////////////////////////////////////////////////////////////////////////////
// $Id: ResponseTimeBox.cpp 4731 2014-07-03 15:18:03Z mellinger $
...
ResponseTimeBox::Initialize()
{
  ...
  if (!res)
    bcierr << "Failed to connect to ResponseTimeBox. Ensure box is connected properly." << std::endl;
    return;
  Sleep(2);
  char buffer[255];
  ...
  if (!res1)
    bcierr << "Failure to retrieve button state information and set device to advanced mode. It is recommended to check connection." << std::endl;
    return;
  ...
  int bytes_read = mDevice->readSerialPort(buffer, sizeof(buffer));
  if (bytes_read == 0)
    bcierr << "Failure to retrieve device ID. Check connection and try again." << std::endl;
    return;
  ...
  if (!res2)
    bcierr << "Failure in communicating with EEPROM. Check connection and try again." << std::endl;
    return;
  Sleep(2);
  char buffer1[255];
  ...
  if (!res3)
    bcierr << "Failure to set TTL width. Check connection and try again." << std::endl;
    return;
  Sleep(2);
  char buffer2[255];
...
ResponseTimeBox::OnExecute()
{
    ...
    while (!Terminating())
    {
        ...
        if (!res11)
            bcierr << "Unable to collect button states. Check connection and start again." << std::endl;
        Sleep(2);
